...
**Cache database file permissions.** On Unix-like systems, `src/db.rs::Database::open` atomically creates the cache file with mode `0600` (owner read/write only) via `OpenOptions::mode` *before* `rusqlite::Connection::open` runs, eliminating any window during which the cached page metadata is world-readable. If the file already exists with looser permissions, a startup warning is logged naming the path and observed octal mode but the file is left untouched — operators who intentionally share it (symlinked debug mount, group-readable for an inspection tool) are not surprised, and everyone else can fix it manually with `chmod 600`. The SQLite WAL auxiliary files (`<path>-wal` and `<path>-shm`) are created by SQLite itself at runtime under the process `umask`; if those also need to be owner-only, set `umask 0077` in the systemd service unit or launcher script. On Windows the helper is a no-op that emits a single process-wide `info` log at startup — no automated ACL enforcement is applied. Windows operators should place the file under `%LOCALAPPDATA%\telegraph-hub-rs\` (or another user-profile directory) where the default NTFS ACL grants only the current user. The detection predicate (`detect_loose_permissions`) is exposed as a module-private function so unit tests can assert the permission policy in isolation while `existing_permissive_file_triggers_warning_log` proves that `Database::open` wires it to a literal `tracing::warn!` call via a thread-scoped `tracing_subscriber::fmt` buffer.
